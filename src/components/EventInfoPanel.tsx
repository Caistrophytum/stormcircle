  ...
  return (
    <div className="flex gap-2 transition-all duration-300 ease-in-out w-fit">
      ...
      <div className="glass-panel p-2.5 whitespace-nowrap">
        <h3 className="text-[16px] font-mono text-primary tracking-[0.2em] uppercase mb-2">
          Top 5 Hazards
        </h3>
          ...
          {topHazards.map((h, i) => (
            <div key={h.hazard} className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-1.5">
                <span className="text-[16px] font-mono text-muted-foreground w-4">
                  {i + 1}.
                </span>
                <span className="text-[17.5px] font-mono text-card-foreground tracking-wider">
                  {h.hazard}
                </span>
              </div>
              <span className="text-[17.5px] font-mono text-primary tabular-nums">
                {h.alerts}
              </span>
            </div>
          ))}
          ...
          {dangerousAlerts.map((a, i) => (
            <div key={a.alert} className="flex flex-col gap-0.5">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[17.5px] font-mono text-card-foreground leading-tight">
                  {a.alert}
                </span>
              </div>
              <div className="ml-[22px]">
                <span
                  className={`text-[14.5px] font-mono tracking-widest px-1.5 py-0.5 rounded-sm ${severityColors[a.severity]} text-background font-bold`}
                >
                  {a.severity}
                </span>
              </div>
