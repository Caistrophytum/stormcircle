...
const RadarMiniMap = ({ expanded, onCollapse }: Props) => {
  if (!expanded) {
    // Collapsed: small clickable radar circle + side info
    return (
      <button
        onClick={onCollapse}
        className="glass-panel p-2 flex items-center gap-3 hover:border-primary/50 transition-all cursor-pointer group"
      >
        {/* Radar circle */}
          ...
          <div className="absolute inset-0 rounded-full bg-background/60 overflow-hidden">
            ...
            {[20, 30, 38].map((r) => (
              <div
                ...
                style={{ width: r * 2, height: r * 2, top: 40 - r, left: 40 - r }}
              />
            ))}
            {/* Sweep */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
              <motion.div
                className="absolute w-[38px] h-[1px] origin-left bg-gradient-to-r from-primary/40 to-transparent"
                animate={{ rotate: 360 }}
        ...
        <div className="flex flex-col gap-1">
          {dataNodes.slice(0, 4).map((node) => (
            <div key={node.label} className="flex items-baseline gap-1.5">
              <span className="text-[7px] font-mono text-muted-foreground w-14 text-right">{node.label}</span>
